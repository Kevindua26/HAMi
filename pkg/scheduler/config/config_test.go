...
// Test_InitDevicesWithConfig_PartialFailure tests that when some device initializers fail,
// others still initialize successfully and errors are collected in initErrors.
...
// Test_InitDevicesWithConfig_TypeAssertionSafety tests that passing invalid config types
// to device initializers returns proper errors instead of panicking.
// The initializers use: cfg.(DeviceConfig) pattern with ok check.
...
// Test_InitDevicesWithConfig_MultipleFailures tests that multiple simultaneous
// device initialization failures are correctly aggregated in initErrors.
...
// Test_InitDevicesWithConfig_IluvatarFailure tests that invalid IluvatarConfig
// is handled gracefully in the Iluvatar device initialization loop.
